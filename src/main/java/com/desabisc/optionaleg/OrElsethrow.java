...
/**
 * 4. orElseThrow() (No Arguments) Real-World Example: ConfigLoader Scenario: Throwing an Exception
 * if a Required Value is Missing In certain cases, you may want to throw an exception if a required
 * value is missing. For example, if you are retrieving an essential configuration setting that must
 * be present, you can use orElseThrow() to throw an exception when it’s absent.
 */
